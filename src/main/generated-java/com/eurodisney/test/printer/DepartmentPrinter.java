/*
 * (c) Copyright 2005-2014 JAXIO, http://www.jaxio.com
 * Source code generated by Celerio, a Jaxio product
 * Want to purchase Celerio ? email us at info@jaxio.com
 * Follow us on twitter: @springfuse
 * Documentation: http://www.jaxio.com/documentation/celerio/
 * Template pack-backend-jpa:src/main/java/printer/Printer.e.vm.java
 */
package com.eurodisney.test.printer;

import java.util.Locale;

import javax.inject.Named;
import javax.inject.Singleton;

import com.eurodisney.test.domain.Department;
import com.eurodisney.test.domain.Department_;
import com.eurodisney.test.printer.support.GenericPrinter;

/**
 * {@link GenericPrinter} for {@link Department} 
 *
 * @see GenericPrinter
 * @see TypeAwarePrinter
 */
@Named
@Singleton
public class DepartmentPrinter extends GenericPrinter<Department> {
    public DepartmentPrinter() {
        super(Department.class, Department_.name);
    }

    @Override
    public String print(Department department, Locale locale) {
        if (department == null) {
            return "";
        }
        StringBuilder ret = new StringBuilder();
        appendIfNotEmpty(ret, department.getName());
        return ret.toString();
    }
}